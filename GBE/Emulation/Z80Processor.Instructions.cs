...
namespace GBE.Emulation
{
    partial class Z80Processor
    {
        ...
        public int Step()
        {
            ...
            {
                Instruction nextInstruction = instructionSet[md[PC++]];

                // Load immediate operand if necessary
                int immediate = 0;
                if (nextInstruction.OperandLength > 0)
                {
                    immediate = nextInstruction.OperandLength > 1 ? LoadWord(PC) : md[PC];
                    PC += nextInstruction.OperandLength;
                }

                // Execute instruction
                nextInstruction.Operation?.Invoke(immediate);

                timer.Increment(nextInstruction.Ticks);
            }
        ...
        byte RotateLeftWithCarry(byte n)
        {
            OpFlag = false;
            HalfCarryFlag = false;
        ...
        byte RotateLeft(byte n)
        {
            ...
            bool oldBit = (n & 0x80) != 0;
            n = (byte)((n << 1) + (CarryFlag ? 1 : 0));
        ...
        byte RotateRightWithCarry(byte n)
        {
            OpFlag = false;
            HalfCarryFlag = false;
        ...
        private void ExtendedOperation(int extOpcode)
        {
            ...
            switch (extOpcode & 0xF0)
            {
                ...
                default:
                    ...
                    {
                        //instruction = "RLC {0}";
                        operation = RotateLeftWithCarry;
                    }
                    else
                    {
                        //instruction = "RRC {0}";
                        operation = RotateRightWithCarry;
                    }
                    break;
                case 0x10:
                    if ((extOpcode & 15) < 8)
